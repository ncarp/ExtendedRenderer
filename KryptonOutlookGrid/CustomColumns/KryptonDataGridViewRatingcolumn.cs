...
namespace AC.ExtendedRenderer.Toolkit.KryptonOutlookGrid.CustomsColumns
{
    /// <summary>
    ...
    public class RatingCell : DataGridViewImageCell
    {
      ...
      /// <param name="e">A DataGridViewCellMouseEventArgs that contains the event data.</param>
        ...
        static RatingCell()
        {
            ...
            // load normal stars 
            for (int i = 0; i <= 5; i++)
                starImages[i] = (Image)AC.ExtendedRenderer.Toolkit.Properties.Resources.ResourceManager.GetObject("star" + i.ToString());

            // load hot normal stars 
            for (int i = 0; i <= 5; i++)
                starHotImages[i] = (Image)AC.ExtendedRenderer.Toolkit.Properties.Resources.ResourceManager.GetObject("starhot" + i.ToString());
        }
        #endregion
